...
- `src/models/options.ts`: reasoning-effort schema, precedence, and CrofAI request payload translation.
...
- Requests must use the fixed CrofAI endpoint and user agent from `src/transport/protocol.ts`; never redirect credentials through a workspace-configurable endpoint.
- Treat CrofAI and OpenAI-compatible response behavior as an undocumented integration surface. Parse defensively and keep protocol-specific behavior covered by tests.
