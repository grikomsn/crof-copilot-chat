...
export const MODELS_DEV_CACHE_KEY = "crofCopilot.modelsDevMetadata.v1";
...
export function normalizeModelsDevSnapshot(payload: unknown, fetchedAt: number): ModelsDevSnapshot {
  const models = record(record(record(payload)?.crof)?.models);
  if (!models) throw new Error("Models.dev returned no Crof model catalog");
  ...
  if (!Object.keys(normalized).length) throw new Error("Models.dev returned no usable Crof models");
