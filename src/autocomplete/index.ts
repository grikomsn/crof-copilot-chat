 ...
 * once at activation; toggling `crofCopilot.inlineSuggestions` is honored on
 * the fly.
 */

import * as vscode from "vscode";
import { ChatCompletionEngine } from "./engine";
import { CrofInlineCompletionProvider } from "./provider";
...
import { crofHeaders, CROF_ENDPOINTS, extensionUserAgent } from "../transport/protocol";
...
export function registerInlineCompletions(context: vscode.ExtensionContext, deps: InlineCompletionsDeps): vscode.Disposable {
  ...
  const engine: CompletionEngine = {
    ...
    async complete(ctx: CompletionContext, signal: AbortSignal): Promise<CompletionResult> {
      ...
      if (!apiKey) {
        log("[completions] no CrofAI API key — skipping");
        return { text: undefined, durationMs: 0 };
      }
      const keyed = new ChatCompletionEngine({
        url: CROF_ENDPOINTS.chat,
        headers: crofHeaders(apiKey, "text/event-stream, application/json", extensionUserAgent(deps.version, deps.vscodeVersion)),
  ...
  const provider = new CrofInlineCompletionProvider({
    engine,
