...
export function validateStreamCompletion(finishReason: string | undefined): void {
  ...
  if (!finishReason) throw new Error("CrofAI response stream ended before a completion reason was received");
  if (finishReason === "length") throw new Error("CrofAI response reached its output token limit before completing");
  if (finishReason === "content_filter") throw new Error("CrofAI stopped the response because of its content filter");
  throw new Error(`CrofAI response ended with finish reason: ${finishReason}`);
}
...
function completeToolCall(tool: PendingToolCall): PendingToolCall {
  ...
  try {
    ...
    throw new Error(`CrofAI response stream ended with incomplete arguments for tool ${tool.name}`);
  }
