...
test("declares native API-key configuration without a management-command override", () => {
  ...
  const provider = manifest.contributes.languageModelChatProviders.find((item) => item.vendor === "crof");
  assert.ok(provider);
...
test("keeps the legacy management commands available for the Secret Storage key", () => {
  ...
  assert.match(
    manifest.contributes.commands.find((item) => item.command === "crofCopilot.testConnection")?.title ?? "",
    /Test Inference/,
  );
