...
export function registerCommands(
  ...
  return [
    ...
    vscode.commands.registerCommand("crofCopilot.setInlineSuggestionsModel", () => setInlineSuggestionsModel()),
...
async function setInlineSuggestionsModel(): Promise<void> {
  ...
  const picked = await vscode.window.showQuickPick<InlineModelPickItem>([
    ...
    { label: "$(pencil) Use a custom model id…", detail: "Enter any CrofAI model id that completes cleanly at reasoning_effort none.", action: "custom" as const },
  ], {
    title: "CrofAI — Set Inline Suggestions Model",
  ...
  if (picked.action === "custom") {
    const value = await vscode.window.showInputBox({
      ...
      value: current,
      prompt: "Any CrofAI model id; the vetted list is a starting point, not a restriction.",
    });
    if (value === undefined || !value.trim()) return;
    await configuration.update(INLINE_SUGGESTIONS_MODEL_SETTING, value.trim(), vscode.ConfigurationTarget.Global);
    void vscode.window.showInformationMessage(`CrofAI inline suggestions model set to ${value.trim()}.`);
    return;
  }
  await configuration.update(INLINE_SUGGESTIONS_MODEL_SETTING, picked.action, vscode.ConfigurationTarget.Global);
  void vscode.window.showInformationMessage(`CrofAI inline suggestions model set to ${picked.action}. Applies on the next keystroke.`);
}
